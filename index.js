...
io.on('connection', socket => {
  ...
  socket.on('set-username', (username, callback) => {
    ...
    if (!same_username && users[socket.id].username == socket.id) {
      console.log(`${users[socket.id].username} set their username to ${username}`);
      users[socket.id].username = username;
      callback(true);
      socket.emit('matchmaking-begin');
      matchMaker(socket.id);
    } else {
      console.log(`${users[socket.id].username} tried to set their username to ${username}, however the username is in use or invalid.`);
    }
  });
  ...
  socket.on('disconnect', () => {
    ...
    // Removes user from the current game and notifies the other player
    for (key in games) {
      let game = games[key];
...
// Very simple matchmaking, as soon as there's two people in the queue, it matches them together
...
function matchMaker(new_player) {
  if (matchmaking.length != 0) {
    ...
    // This is all completely unneeded but may be useful for future additions to the game
    ...
    // Tells players that a game has started - allows the client to initialize the view
